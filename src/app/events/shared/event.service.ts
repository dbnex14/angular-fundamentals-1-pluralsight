...
/*
@Injectable() decorator is not realy required for this service because it is only
required when you inject a service that injects other services as dependencies on
its own.  That is the case when we have a constructor that injects services like
constructor(private http: Http){}, so in this case EventService injects Http service
and in such case, @Injectable() decorator is required.  But since you never know if
a service will inject another service later, it is best practice to just decorate it
right away with @Injectable.
*/
@Injectable()
